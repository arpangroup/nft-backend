...
public class RankDataInitializer {
    ...
    public void init() {
        // Rank 1: No level requirement
        RankConfig rank1 = new RankConfig(RANK_1, 0, 0, new BigDecimal("5"));//5%

        // Rank 2: Level A = 3, Level B = 4, Level C = 1
        RankConfig rank2 = new RankConfig(RANK_2, 0, 0, new BigDecimal("10"));//10%
        rank2.getRequiredLevelCounts().put(1, 1); // Level A
        rank2.getRequiredLevelCounts().put(2, 1); // Level B
        rank2.getRequiredLevelCounts().put(3, 1); // Level C

        // Rank 3: Level A = 6, Level B = 18, Level C = 2
        RankConfig rank3 = new RankConfig(RANK_3, 0, 0, new BigDecimal("15"));//15%
        rank3.getRequiredLevelCounts().put(1, 1);
        rank3.getRequiredLevelCounts().put(2, 1);
        rank3.getRequiredLevelCounts().put(3, 1);

        // Rank 4: Level A = 15, Level B = 30, Level C = 5
        RankConfig rank4 = new RankConfig(RANK_4, 0, 0, new BigDecimal("20"));
        rank4.getRequiredLevelCounts().put(1, 1);
        rank4.getRequiredLevelCounts().put(2, 1);
        rank4.getRequiredLevelCounts().put(3, 1);
